...
### Medicine Dispensing Appointment
A scheduled appointment window required by healthcare institutions (EPS / pharmacy networks) for patients to pick up controlled and essential prescription medication in person at physical dispensary branches.
...
2. **Rule 2 (Availability Status)**: The slot's `status` or `state` field must explicitly equal `'free'` or `'available'` (or localized provider status `'disponible'`).
...
### CAPTCHA Fallback Guardrail (`P-ACT-04`)
...
### Circuit Breaker Pattern (`P-NET-03`)
...
### Mutex Lock (Concurrency Lock, `P-ACT-02`)
...
An operational philosophy where the robot continuously monitors and evaluates data, but critical healthcare decisions (selecting which family member books the slot) remain under explicit human control via 1-click mobile prompts.
...
### Personally Identifiable Information (PII) Masking (`P-SEC-02`)
...
The collection of non-negotiable safety policies governing security (`P-SEC`), networking (`P-NET`), business integrity (`P-BIZ`), browser action (`P-ACT`), and developer workflows (`P-DEV`).
